...
k_x, k_y = 0.6, 0.3
speed = 0.12
body_kp = 0.5  # Proportional gain for body alignment
body_kd = 0.17  # Derivative gain for body alignment
prev_yaw_angle = 0.0  # Initialize previous yaw angle
min_distance = 0.1  # Minimum distance threshold (to be tuned experimentally)
step_distance = 0.2  # Distance to move forward in each step
walking_kp = 0.05 # Proportional gain for approaching person
...
def align_body_with_head(motion_proxy):
    """
    ...
    """
    global prev_yaw_angle

    # Get the current head yaw angle
    head_yaw_angle = motion_proxy.getAngles("HeadYaw", True)[0]

    # Stop the body movement if the head yaw angle is within the threshold
    if abs(head_yaw_angle) < 0.2:
        #print(f"Head yaw angle ({head_yaw_angle:.2f}) within threshold. Stopping body alignment.")
...
def walk_to_person(motion_proxy, box, goal_distance = 110, min_distance = 10):
    """
    Walk towards the person based on bounding box size.
    """
    
    if box is None:
        print("No detections found. Cannot walk forward.")
        return
    ...
    if abs(distance_error) < min_distance:
        print("Person is within minimum distance. Stopping.")
        motion_proxy.move(0, 0, 0)
        #make_robot_speak(session, "Robot finished moving")
        person_approached = True
        return
    walking_speed = float(np.clip(walking_kp * distance_error, -0.08, 0.08))

    #print(f"Walking towards person. Estimated distance: {distance_estimate:.2f}")
...
def video(session, motion_proxy):
    ...
    id = video.subscribe("teamm21", RESOLUTION, COLOR_SPACE, 10)
    ...
    while True:
        ...
        if max_class_id == 2:  # 'standing'
            ...
            walk_to_person(motion_proxy, box)
            # if person_approached == True:
            #     set_led_color(session, 0x800080) #Purple
